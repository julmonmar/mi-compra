...
const PLATOS_INIT = [
  ...
  {id:"pl17", nombre:"Bizcocho de limón",      tipo:"extra",    prods:[{pid:"p11",c:3},{pid:"p22",c:0.1}]},
];

const SEMANAS_INIT = [
  {id:"s1",nombre:"Semana A",color:"#C8A96E",almuerzos:["pl1","pl2","pl3","pl4","pl5"],cenas:["pl10","pl11","pl12","pl13"],extras:["pl17"]},
  {id:"s2",nombre:"Semana B",color:"#7EB8A4",almuerzos:["pl6","pl7","pl8","pl9","pl2"],cenas:["pl14","pl15","pl16","pl10"],extras:[]},
  {id:"s3",nombre:"Semana C",color:"#C47E7E",almuerzos:["pl1","pl4","pl5","pl8","pl6"],cenas:["pl10","pl13","pl14","pl16"],extras:[]},
];
...
function calcMapaMenu(semana, platos, productos, quitados, extrasAct=[]) {
  const mapa = {};

  // 1. Sumar productos del menú activo (almuerzos + cenas + extras activados)
  const activos = [
    ...(semana?.almuerzos||[]).filter(id=>!quitados.includes(id+"|a")),
    ...(semana?.cenas||[]).filter(id=>!quitados.includes(id+"|c")),
    ...(semana?.extras||[]).filter(id=>extrasAct.includes(id)),
  ];
...
export default function App() {
  ...
  const [extrasAct,     setExtrasAct]     = useState([]); // ids de extras activados
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, snap => {
      ...
      if (snap.exists()) {
        ...
        if (d.extrasAct)  setExtrasAct(d.extrasAct);
        ...
        const inicial = {
          ...
          semanas:SEMANAS_INIT, idxSemana:0, quitados:[], extrasAct:[],
          checks:{}, listaSuper:[],
        };
        setDoc(ref, inicial);
      }
  ...
  useEffect(() => {
    if (!cargado) return;
    guardarFirebase({ pantalla, productos, platos, semanas, idxSemana, quitados, extrasAct, checks, listaSuper });
  }, [pantalla, productos, platos, semanas, idxSemana, quitados, extrasAct, checks, listaSuper, cargado]);
  ...
  function rotarSemana() {
    ...
    const nuevoEstado = {
      ...
      idxSemana: nuevaIdx, quitados:[], extrasAct:[], checks:{}, listaSuper:[],
    };
    guardando.current = true;
  ...
  function irAlSuper() {
    ...
    const mapaMenu = calcMapaMenu(semana, platos, productos, quitados, extrasAct);
    const lista = [];
  ...
  return (
    <Lista
      semana={semana} semanas={semanas} setSemanas={setSemanas} idxSemana={idxSemana}
      platos={platos} productos={productos}
      quitados={quitados} quitarPlato={quitarPlato}
      extrasAct={extrasAct} setExtrasAct={setExtrasAct}
...
// PANTALLA LISTA
// ══════════════════════════════════════════════════════════════════════════════
function Lista({ semana, semanas, setSemanas, idxSemana, platos, productos, quitados, quitarPlato,
  extrasAct, setExtrasAct,
      ...
      <div style={S.scroll}>
        ...
        {tab==="menu" && (
          <TabMenu semana={semana} platos={platos} productos={productos} quitados={quitados}
            quitarPlato={quitarPlato} color={color} rotarSemana={rotarSemana}
            extrasAct={extrasAct} setExtrasAct={setExtrasAct}
            semanas={semanas} setSemanas={setSemanas} idxSemana={idxSemana}/>
        )}
        {/* ── TAB LISTA ── */}
        {tab==="lista" && (
...
// ── TAB MENÚ ──────────────────────────────────────────────────────────────────
function TabMenu({ semana, platos, productos, quitados, quitarPlato, color, rotarSemana, extrasAct, setExtrasAct, semanas, setSemanas, idxSemana }) {
  const [modal, setModal] = useState(null); // "almuerzo" | "cena" | "extra"
  ...
  const extrasAsignados = semana?.extras||[];

  function toggleEnSemana(platId, seccion) {
    const campo = seccion==="extra" ? "extras" : seccion==="almuerzo" ? "almuerzos" : "cenas";
    setSemanas(s=>s.map((se,i)=>{
      if(i!==idxSemana)return se;
      const arr=se[campo]||[];
      return{...se,[campo]:arr.includes(platId)?arr.filter(x=>x!==platId):[...arr,platId]};
    }));
  }

  // Lista de platos para el modal
  function platosModal(seccion) {
    if (seccion==="extra") return platos.filter(p=>p.tipo==="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es"));
    const tipo = seccion;
    return [
      ...platos.filter(p=>p.tipo===tipo && p.tipo!=="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es")),
      ...platos.filter(p=>p.tipo!==tipo && p.tipo!=="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es")),
    ];
  }

  function idsActuales(seccion) {
    if(seccion==="extra") return extrasAct; // activos, no asignados
    if(seccion==="almuerzo") return semana?.almuerzos||[];
    return semana?.cenas||[];
  }

  function handleModalClick(platId, seccion) {
    if (seccion==="extra") {
      const estaAsignado = extrasAsignados.includes(platId);
      if (estaAsignado) {
        // Quitar: eliminar de asignados Y de activos
        setSemanas(s=>s.map((se,i)=>{
          if(i!==idxSemana)return se;
          return{...se,extras:(se.extras||[]).filter(x=>x!==platId)};
        }));
        setExtrasAct(e=>e.filter(x=>x!==platId));
      } else {
        // Añadir: asignar y activar
        setSemanas(s=>s.map((se,i)=>{
          if(i!==idxSemana)return se;
          return{...se,extras:[...(se.extras||[]),platId]};
        }));
        setExtrasAct(e=>[...e,platId]);
      }
    } else {
      toggleEnSemana(platId, seccion);
    }
  }
    ...
    <div style={{padding:"18px 20px 0"}}>
      ...
      {modal && (
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.75)",zIndex:100,
          display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:152}}
          onClick={()=>setModal(null)}>
          <div onClick={e=>e.stopPropagation()}
            style={{background:"#161412",border:"1px solid #252220",borderRadius:14,
              width:"calc(100% - 32px)",maxWidth:480,maxHeight:"65vh",display:"flex",flexDirection:"column",
              padding:"0 0 16px",boxShadow:"0 8px 32px rgba(0,0,0,0.6)"}}>
            {/* Header modal */}
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",
              padding:"16px 20px 12px",borderBottom:"1px solid #1E1C18"}}>
              <span style={{fontSize:15,fontWeight:"bold",color:"#F0EAD6"}}>
                {modal==="almuerzo"?"☀️ Añadir almuerzos":modal==="cena"?"🌙 Añadir cenas":"✨ Añadir extras"}
              </span>
            ...
            <div style={{overflowY:"auto",padding:"8px 20px"}}>
              {platosModal(modal).length===0 && (
                <div style={{color:"#444",fontSize:13,padding:"16px 0",textAlign:"center"}}>
                  No hay {modal==="extra"?"extras":"platos"} creados aún.
                </div>
              )}
              {platosModal(modal).map((pl, i, arr)=>{
                const enLista = idsActuales(modal).includes(pl.id);
                const esTipoPropio = pl.tipo === modal;
                const anteriorEsPropio = i > 0 && arr[i-1].tipo === modal;
                const esPrimeroDelOtro = !esTipoPropio && anteriorEsPropio;
                return(
                  <div key={pl.id}>
                    {esPrimeroDelOtro && (
                      <div style={{borderTop:"1px solid #252220",margin:"8px 0"}}/>
                    )}
                    <div onClick={()=>handleModalClick(pl.id,modal)}
                      style={{...S.itemRow,opacity:enLista?1:0.5,marginBottom:4}}>
                      <div style={{...S.check,background:enLista?color:"transparent",borderColor:enLista?color:"#333"}}>
                        {enLista&&<span style={{color:"#0F0E0C",fontSize:10,fontWeight:"bold"}}>✓</span>}
                      </div>
                      <span style={{flex:1,fontSize:15,color:enLista?"#F0EAD6":"#777"}}>{pl.nombre}</span>
                      {!esTipoPropio && <span style={{fontSize:12,color:"#444",marginLeft:4}}>{pl.tipo}</span>}
                    </div>
                  </div>
                );
              })}
      ...
      {/* Almuerzos y Cenas */}
      {[["almuerzo","☀️ Almuerzos",semana?.almuerzos||[]],
        ...
        <div key={tipo} style={{marginBottom:22}}>
          ...
          <button onClick={()=>setModal(tipo)}
            style={{background:"transparent",border:"none",color:"#555",
              fontSize:12,cursor:"pointer",padding:"6px 0",marginTop:2}}>
            + añadir
          </button>
        </div>
      ))}
      ...
      <div style={{marginBottom:22}}>
        <div style={S.catHeaderEl}>
          <span>✨ Extras</span>
          <span style={{marginLeft:"auto",fontSize:11,color:"#555"}}>
            {extrasAsignados.filter(id=>extrasAct.includes(id)).length} activos
          </span>
        </div>

        {extrasAsignados.map(platId => {
          const plato = platos.find(p=>p.id===platId);
          if (!plato) return null;
          const activo = extrasAct.includes(platId);
          return (
            <div key={platId} onClick={()=>setExtrasAct(e=>
              activo ? e.filter(x=>x!==platId) : [...e, platId]
            )}
              style={{...S.platoCard,opacity:activo?1:0.32,
                borderColor:activo?`${color}40`:"#1E1C18",
                background:activo?"#1C1A17":"#141312"}}>
              <div style={{...S.dot,background:activo?color:"#2a2a2a"}}/>
              <span style={{flex:1,fontSize:15,color:activo?"#F0EAD6":"#444",
                textDecoration:activo?"none":"line-through"}}>{plato.nombre}</span>
              <span style={{fontSize:11,color:activo?"#555":"#333"}}>
                {activo?`${plato.prods.filter(pr=>productos.find(p=>p.id===pr.pid)).length} prod.`:"quitado"}
              </span>
            </div>
          );
        })}
        <button onClick={()=>setModal("extra")}
          style={{background:"transparent",border:"none",color:"#555",
            fontSize:12,cursor:"pointer",padding:"6px 0",marginTop:2}}>
          + añadir
        </button>
...
function SecSemanas({semanas,setSemanas,platos,productos}) {
  ...
  const [sub,setSub]=useState("almuerzos"); // almuerzos | cenas | extras
  ...
  function añadir(){
    ...
    setSemanas(s=>[...s,{id:`s${Date.now()}`,nombre:`Semana ${l}`,
      color:COLORES[semanas.length%COLORES.length],almuerzos:[],cenas:[],extras:[]}]);
  }
  ...
  function togglePlato(semId,platId,sub){
    const campo = sub==="extras" ? "extras" : sub==="almuerzos" ? "almuerzos" : "cenas";
  ...
  const filtrados=(sub)=>{
    if(sub==="extras") return platos.filter(p=>p.tipo==="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es"));
    // Para almuerzos y cenas: primero los del tipo, luego el resto (orientativo)
    const tipo = sub==="almuerzos"?"almuerzo":"cena";
    const principal = platos.filter(p=>p.tipo===tipo && p.tipo!=="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es"));
    const otros = platos.filter(p=>p.tipo!==tipo && p.tipo!=="extra").sort((a,b)=>a.nombre.localeCompare(b.nombre,"es"));
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <div style={{flex:1,cursor:"pointer"}} onClick={()=>setOpen(o=>o===sem.id?null:sem.id)}>
              ...
              <div style={{color:"#555",fontSize:11,marginTop:2}}>
                {sem.almuerzos?.length||0} almuerzos · {sem.cenas?.length||0} cenas · {sem.extras?.length||0} extras
              </div>
            </div>
          ...
          {open===sem.id&&(
            <div style={{marginTop:14,borderTop:"1px solid #1E1C18",paddingTop:14}}>
              <Tabs tab={sub} setTab={setSub} color={sem.color}
                tabs={[["almuerzos","☀️ Almuerzos"],["cenas","🌙 Cenas"],["extras","✨ Extras"]]} small/>
              <div style={{marginTop:10}}>
                {filtrados(sub).map(pl=>{
                  const arr = sub==="almuerzos"?sem.almuerzos:sub==="cenas"?sem.cenas:(sem.extras||[]);
                  const activo = arr.includes(pl.id);
                  return(
                    <div key={`${pl.id}-${sub}`} onClick={()=>togglePlato(sem.id,pl.id,sub)}
...
Receta: ${ia.texto}`;
  ...
  return(
    <div>
      ...
      {[["almuerzo","☀️ Almuerzos"],["cena","🌙 Cenas"],["extra","✨ Extras"]].map(([tipo,label])=>(
